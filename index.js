...
1. Ответь вежливо: "Сізбен біздің әкімшілік тікелей байланысады. / По вопросам трудоустройства и вакансий с вами напрямую свяжется наш администратор."
...
async function handleIncomingMessage(body) {
  ...
  // Если это запрос о трудоустройстве или вакансиях
  if (isJobInquiry) {
    await saveMessageToDB(chatId, 'system', '[AI_DISABLED]');
    console.log(`💼 Запрос о работе от чата [${chatId}]!`);
    if (ADMIN_PHONE) {
      const guestPhone = chatId.split('@')[0] || 'неизвестен';
      const adminAlertText = 
        `💼 **ЗАПРОС О РАБОТЕ / ВАКАНСИИ — "Гранд Вилла"**\n\n` +
        `👤 Кандидат интересуется вакансиями или трудоустройством!\n` +
        `📞 Номер телефона кандидата: +${guestPhone}\n\n` +
        `⚠️ ИИ остановил переписку. Пожалуйста, свяжитесь с кандидатом напрямую!`;
